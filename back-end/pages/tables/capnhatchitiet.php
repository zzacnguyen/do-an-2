...
if (isset($_POST['submit'])) {
    ...
    if (! empty($tensp) && ! empty($mahinh) && ! empty($tinhtrang) && ! empty($bosuutap) && ! empty($series) && ! empty($kichthuoc) && ! empty($chatlieu) && ! empty($khung) && ! empty($loaimay) && ! empty($loaidaydeo) && ! empty($matquayso) && ! empty($chongnuoc) && ! empty($gioitinh) && ! empty($hienthi) && ! empty($mota)) {
        $sql = $sql->update('chitietsp', array(
            'SP_ID' => $tensp,
            'HA_ID' => $mahinh,
        ...
        echo '<script>alert("Thông tin cập nhật không được bỏ trống");</script>';
    }
}
?>
...
<section class="content-header">
	<h1>
		Cập nhật chi tiết sản phẩm <small></small>
	</h1>
	<ol class="breadcrumb">
		<li><a href="#"><i class="fa fa-dashboard"></i>Trang chủ</a></li>
		<li><a href="#">Sản phẩm</a></li>
		<li class="active">Cập nhật chi tiết sản phẩm</li>
	</ol>
</section>
					...
					<div class="box-body">
						...
						<div class="form-group">
							...
							<textarea class="form-control" rows="3"
								placeholder="Nhập nội dung ..." name="mota"><?php echo $query['CTSP_MOTA'] ?></textarea>
						</div>
					</div>
					<!-- /.box-body -->
